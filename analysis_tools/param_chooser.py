...
class AlgoParamResults:
    ...
    def get_best_params(self, even_money):
        if even_money:
            self.pnl_df = self.pnl_df[self.pnl_df['atrStopLossPercents'] == self.pnl_df['atrTakeProfitPercents']]

        best_params = []
        for side in ['Bull', 'Bear']:
            temp_side = self.pnl_df[self.pnl_df['side'] == side].copy(deep=True)
            temp_side = temp_side[temp_side['tot_trades'] > 400]
            temp_side.drop_duplicates(subset=['side', 'PnL'], inplace=True)

            if len(temp_side) <= 10:
                best_params.append(temp_side)
            else:
                temp_side = temp_side.sort_values(['PnL', 'atrTakeProfitPercents'], ascending=[False, False])
                temp_side.reset_index(drop=True, inplace=True)
                temp_ema = temp_side.groupby('atrTakeProfitPercents').head(2)
    ...
    def get_other_params(self):
        ...
        for side in ['Bear', 'Bull']:
            ...
            temp_side = temp_side.sort_values(['PnL', 'atrTakeProfitPercents'], ascending=[False, False])
            temp_side.reset_index(drop=True, inplace=True)
            for i in list(range(1, 13, 2)) + [2]:
                other_params.append(pd.DataFrame(temp_side.iloc[int(1 / i * len(temp_side))-1]).T)

            worst_params.append(pd.DataFrame(temp_side.iloc[:-5]).T)
    ...
    def save_all_params(self, valid_params, side):
        self.params_save_file = \
            f'{self.trade_folder}\\{self.ph.setup_params.security}_{self.ph.setup_params.time_frame_test}'
    ...
    def run_param_chooser(self, even_money):
    ...
    def set_lstm_nodes(self, ind, model_increase):
        ...
        for layer in ['lstm_i1_nodes', 'lstm_i2_nodes', 'dense_m1_nodes', 'dense_wl1_nodes', 'dense_pl1_nodes']:
            ...
            self.pnl_df[layer] = (self.pnl_df[layer] * size_adj).astype(int)

    def adj_lstm_training_nodes(self, ind, model_increase=0):
        self.set_lstm_nodes(ind, model_increase)
        for layer in ['lstm_i1_nodes', 'lstm_i2_nodes', 'dense_m1_nodes', 'dense_wl1_nodes', 'dense_pl1_nodes']:
    ...
    def valid_param_list(self, train_bad_paramsets):
        ...
        valid_params = np.unique([int(p) for p in valid_params])
